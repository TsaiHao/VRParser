...
void VibrationExperiment::splitEmgByMarkers(const string &outDir) {
    if (_parsers.find("emg") == _parsers.end()) {
        return;
    }

    auto& parser = _parsers["emg"];
    EmgParser* p = dynamic_cast<EmgParser*>(parser.get());
    p->reduceChannel(7);
    if (!utils->exists(outDir)) {
        utils->createDirectory(outDir);
    }
    char buffer[1000];
    const int LENGTH = 20;
    const char *fmt = "emg_%02d_%s_%02d_%s";
    int i = 0;

    for (auto &m : _markers.Markers()) {
        size_t beg = m.getCounter("emg");
        float* data = nullptr;
        // TODO: offset
        int n = parser->getDataByColumn(&data, beg + 800, beg + 800 + LENGTH * parser->samplingRate());
        snprintf(buffer, 1000, fmt,
                 subject(), vib().c_str(), trail(), bias()[i].toString().c_str());
        auto fn = outDir / fs::path(buffer);
        ofstream ofs(fn, ios::out | ios::binary);
